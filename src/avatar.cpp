...
void AvatarController::computeSlow()
{
    ...
    else if (rd_.tc_.mode == 11)
    {
        ...
        ///////////////////////////////FINAL TORQUE COMMAND/////////////////////////////
        torque_desired_prev_ = torque_sum_;
        rd_.torque_desired   = torque_sum_;
        ///////////////////////////////////////////////////////////////////////////////
    }
...
{   
    ...
    if(current_step_num_mpc_== 0 || walking_tick_mpc_ - com_start_tick_mpc_ < 2*t_total_mpc_ - hz_/thread3_hz_)
    {
...
void AvatarController::econom2_thread_stepchange()
{
    ...
    if((walking_tick_mpc_ - (t_start_mpc_ + 1*t_total_mpc_) >= -hz_/thread3_hz_) && (current_step_num_mpc_ < total_step_num_ - 1))
    {
...
void AvatarController::updateInitialState()
{
    if (walking_tick_ == 0)
    {
        ...
        lfoot_float_init_.translation()(1) = 0.1025 + 0.0*(bool)(scenario_num_);
        ...
        rfoot_float_init_.translation()(1) = -0.1025 + 0.0*(bool)(scenario_num_);
...
void AvatarController::addZmpOffset()
{
    ...
    if(mpc_on_bool_)
    {
        ...
        rfoot_zmp_offset_ =  (0.055 - 0.015*(1 - (bool)current_step_num_));
    }
...
void AvatarController::zmpGenerator(const unsigned int norm_size, const unsigned planning_step_num)
{
    ...
    if (current_step_num_ == 0)  
    {
        ...
        {
            ref_vrp_.block(2.0*hz_, 2, t_temp_ - 2.3*hz_, 1).setConstant(zc_mj_ - 0.10*param_loco_manipulation_);
        }
...
void AvatarController::getFootTrajectory()
{
    ...
    //if(abs(del_F_(0)*del_F_(0) + del_F_(1)*del_F_(1)) > 1e-2)
...
void AvatarController::getComTrajectory_mpc()
{
    ...
    e_tmp_graph17 << MPC_Stabilizer_state_main_.transpose() << endl;
    ...
    e_tmp_graph18 << MPC_Stabilizer_state_main_.transpose() << endl;
...
e_tmp_graph18 << MPC_Stabilizer_state_main_(5) << ","
              << MPC_Stabilizer_state_main_p_(5) << ","
              << MPC_Stabilizer_state_main_i_(5) << ","
              << endl;
...
void AvatarController::IS_FIPM_3D_DCM_Stabililzer_MPC(double mpc_freq, double preview_window)
{
    ...
    step_enable_bool_mpc_          = (bool)((mpc_tick > 0.4*t_temp_)&&(mpc_tick                   < (1 - (bool)(current_step_num_mpc_))*t_temp_ + t_total_const_ - t_dsp2_const_ - step_enable_time_fwd_*hz_ - step_enable_fix_time_pre_*hz_));
    step_enable_bool_one_tick_mpc_ = (bool)((mpc_tick > 0.4*t_temp_)&&(mpc_tick + MPC_synchro_hz_ < (1 - (bool)(current_step_num_mpc_))*t_temp_ + t_total_const_ - t_dsp2_const_ - step_enable_time_fwd_*hz_ - step_enable_fix_time_pre_*hz_));
    ...
    for(int i = 0; i < N_stab_mpc; i++)
    {
        ...
        { 
            ...
            if((walking_tick_mpc_ > 0.4*t_temp_) && (next_step_start_prev_tick < N_stab_mpc))
            {
...
void AvatarController::CP_compen_MJ_FT()
{
    ...
    double Z_F_T_minmaxcut_mj_ft = 0.4;
...
e_tmp_graph1 << vrp_desired_(2) << ","
             ...
             << rfoot_support_current_.translation()(1)    << ","
             << lfoot_trajectory_support_.translation()(1) << ","
             << lfoot_support_current_.translation()(1)    << ","
             << alpha2 << ","
             << alpha_weight_cubic << ","
             << rfoot_trajectory_support_ideal_.translation()(1) << ","
             << lfoot_trajectory_support_ideal_.translation()(1) << ","
             << endl;
